...
class N_event:
	...
	export_lst = [("n_trans_angle", (100,0,360) ), ("p_trans_angle", (100, 0, 360) ), ("nu_px", (100,-300, 300) ), ("nu_py", (100, -300, 300) )]
	c = 3E8 #m/s


	def __init__(self, filename, export=False, printing=True, export_fname="nparse_default", quiet=False):
		#export_lst has format: [(case, (bins, min, max)), ...]
		current_tree = None
		...
		self.rnge = range(0,40)

		#Exporting
		self.title = export_fname
		self.export = export
		if self.export:
			title_lst = [i[0] for i in self.export_lst]
			rnge_lst = [i[1] for i in self.export_lst]
			hout = R.TFile(self.title+".root", "RECREATE")
			self.export_dct = dict(zip(title_lst, rnge_lst))
			for hname, ranges in self.export_dct.iteritems():
				self.export_dct[hname] = R.TH1F(hname, hname, ranges[0], ranges[1], ranges[2])
				#export_dct[hname].Fill(value)
	...
	#Splits an event into a dct of particles and their 4vecs IN DETECTOR COORDS
		...
		for part_num, nth_dct in part_dct.iteritems():
			if nth_dct["Name"] in ["neutron", "proton"]:
				...
				#self.get_u(v_b)	
				self.compare_vecs(v_b, v_nu)			
				#Export angles to hist; these are NOT one-to-one with the event
				if self.export and name=="neutron" and summary["out"]["neutron"] == 1 and summary["out"]["proton"] == 0:
					self.export_dct["n_trans_angle"].Fill(azi)
	...
	def fetch_vtx(self, event):
	#Return a tuple of the vertex (x, y, z, t)
		leaf_obj = self.treeinfo[1].GetLeaf("mc_vtx")
		mc_vtx = [0, 0, 0, 0]
		#leaf_2 = self.treeinfo[1].GetLeaf("CCQEAntiNuTool_vtx")
		#print "\n",event
	...
	#Pass this v_n*time and vec(vtx->blob) in (m)
		...
		dot = np.dot(a,b)
		#law of cosines
		theta = np.degrees(np.arccos(dot/(norm_a*norm_b)))
...
if __name__ == "__main__":
	N_event("smallmc.root", export=False, printing=False, quiet=True)
